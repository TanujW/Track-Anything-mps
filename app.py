...
def download_checkpoint(url, folder, filename):
    ...
    if not os.path.exists(filepath):
        ...
        print("Found checkpoint file: {}".format(filepath))
...
if "sam_model_type" not in args:
    args.sam_model_type = None

if args.sam_model_type is None:
    available_chkpoints = glob.glob(f"{folder}/*sam*.pth", recursive=True)
    
    if len(available_chkpoints) == 0:
        args.sam_model_type = "vit_h"
    else:
        print("Available models: {}".format(available_chkpoints))
        sam_checkpoint = {v: k for k, v in SAM_checkpoint_dict.items()}[os.path.basename(available_chkpoints[0])]
    
        args.sam_model_type = sam_checkpoint
        
print("Using SAM model: {}".format(args.sam_model_type))
...
with gr.Blocks() as iface:
    ...
    with gr.Row():
        ...
        with gr.Column():
            with gr.Row():
                video_input = gr.Video(scale=0.4)
                with gr.Column():
                ...
                with gr.Column():
                    ...
                    with gr.Row():
                        with gr.Row():
                            ...
                            clear_button_click = gr.Button(value="Clear clicks", interactive=True, visible=False, size="sm")
                            Add_mask_button = gr.Button(value="Add mask", interactive=True, visible=False)
                    template_frame = gr.Image(type="pil",interactive=True, elem_id="template_frame", visible=False, height=360)
